...
- **Auto-generate PINS from the DOM instead of hand-maintaining it** — the `PINS` map-data array is a separately hand-authored JSON blob rather than being derived from the schedule cards themselves. As of 2026-08-16, every non-Hotel PINS entry now carries an `"id"` matching its card's `data-id` (id-linking pass, `app-patch-reviewer` rule W9 enforces it going forward), which stops entries from silently drifting apart — but the array is still two sources of truth that happen to agree today, not one source of truth. The heavier fix (auto-generate `PINS` from the rendered `.card` elements at page-load, eliminating the separate array entirely) is bigger/riskier and deliberately deferred post-trip — don't conflate it with the id-linking fix already done.
- **Trash-can icon parity for custom (added) events** — Hanan raised 2026-08-14: custom events only get a text "Remove" button (`nwBuildCustomCard`, `delBooking`), not the same `rowdel` trash-can icon that static pre-planned cards have. He wants the same hide/restore behavior for every event in the plan, static or custom. Deliberately left as-is for now (not urgent). Overlaps with the two-way-delete idea directly above — if that gets built, custom events should get the same trash-can icon + hide-vs-delete choice as static cards, not just parity on the delete-only "Remove" button.
